...
from toolcv.api.pytorch_lightning.utils import load_callbacks, load_logger, set_seed, predict_yolov1
...
def main(training=True, visual=False):
    ...
    _model = Yolov1simple('dla34', True, num_anchors, num_classes, strides, dropout, 5, freeze_at)
    # _model = Yolov1simpleV2('dla34', True, num_anchors, num_classes, strides,dropout, 5, freeze_at)
    ...
    if os.path.exists(checkpoint_path):
        ...
        if training:
            ...
            _model.load_state_dict({k.replace("model.",'',1):v for k,v in state_dict.items()})
    ...
    if training:
        ...
        predict_yolov1(_model, img_paths, test_transform, resize, device, visual, strides, fix_resize=True, mode=mode,
                       box_norm=box_norm, iou_threshold=0.3, conf_threshold=0.1)
