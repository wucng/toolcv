...
from toolcv.api.pytorch_lightning.utils import load_callbacks, load_logger, set_seed, predict_ssdMS
...
from toolcv.tools.utils import train_ssdMS as train
...
def main(training=True, visual=False):
    ...
    _priorBox = {
        "min_dim": 300.0,
        "min_sizes": [30, 65, 130],
        "max_sizes": [60, 130, 200],
        "aspect_ratios": [[2, 3], [2, 3], [2, 3]],  # [[2],[2],[2],[2]]
        "strides": strides
    }
    anchors = get_prior_boxMS((resize, resize), strides, xywh=False,_priorBox=_priorBox)  # 缩放到0~1
    num_anchors = 6
    ...
    model = SSDsimpleMS('dla34', True, num_anchors, num_classes, strides,dropout,freeze_at).to(device)
    ...
    if training:
        ...
        predict_ssdMS(anchors, model, img_paths, test_transform, resize, device, visual, strides, fix_resize=True,
