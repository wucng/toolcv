...
from toolcv.network.net import Yolov3simple,Yolov3simpleV2
...
from toolcv.api.pytorch_lightning.utils import load_callbacks, load_logger, set_seed, predict_yolov3
...
from toolcv.tools.utils import train_yolov3 as train
...
def main(training=True, visual=False):
    ...
    anchors = [[[0.1, 0.1], [0.125, 0.125], [0.15, 0.15]],  # s =8
               [[0.2, 0.2], [0.225, 0.225], [0.25, 0.25]],  # s =16
               [[0.3, 0.3], [0.325, 0.325], [0.35, 0.35]]]  # s =32 # 缩放到0~1 [w,h]
    ...
    model = Yolov3simple('dla34', True, num_anchors, num_classes, strides,dropout, 5, freeze_at).to(device)
    ...
    if training:
        ...
        dataLoader = DataLoader(train_dataset, 1 if multiscale else batch_size, True, num_workers=0,
        ...
        for epoch in range(epochs):
            train(model, optim, dataLoader, device, epoch, multiscale, mode, reduction='mean')
        ...
        predict_yolov3(anchors, model, img_paths, test_transform, resize, device, visual, strides, fix_resize=True,
                       mode=mode, iou_threshold=0.3, conf_threshold=0.1)
