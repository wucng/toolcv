...
from toolcv.api.pytorch_lightning.utils import load_callbacks, load_logger, set_seed, predict_yolov2
...
from toolcv.tools.utils import train_yolov2 as train
...
def main(training=True, visual=False):
    ...
    anchors = np.array([[0.1, 0.1], [0.125, 0.125], [0.3, 0.3], [0.5, 0.5]])  # 缩放到0~1 [w,h]
    ...
    # _model = Yolov1simpleV2('dla34', True, num_anchors, num_classes, strides,dropout, 5, freeze_at).to(device)
    # if not training:
    ...
    if training:
        ...
        predict_yolov2(anchors, model, img_paths, test_transform, resize, device, visual, strides, fix_resize=True,
