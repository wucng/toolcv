"""
参考 IENet 预测 x,y,w,h 与 确定旋转的 w1,h1
"""
...
from toolcv.tools.utils import train_ienet as train, predict_ienet as predict
...
class FruitsNutsDataset(Dataset):
    ...
    def __init__(self, root="", classes=[], resize=416, strides=32, angle=30,
                 batch_size=32, num_anchors=1, transform=None, mode='fcos',
                 use_mosaic=False, fix_resize=False, imagu=False, advanced=False):
        self.root = root
...
def main(training=True, visual=True):
    ...
    # model = Yolov1simple('resnet34', True, num_anchors, num_classes, strides,0.5,7).to(device)
    model = Yolov1simple('dla34', True, num_anchors, num_classes, strides, 0.5, 7).to(device)
    ...
    if training:
        dataset = FruitsNutsDataset(FILE_ROOT, classes, resize, strides, angle, batch_size,
                                    num_anchors, transform, mode, use_mosaic, fix_resize,imagu,advanced)
        ...
        dataLoader = DataLoader(train_dataset, batch_size, True, collate_fn=dataset.collate_fn)
        ...
        for epoch in range(epochs):
            train(model, optim, dataLoader, device, epoch, mode)
        ...
        for img_path in img_paths:
            info = predict(model, img_path, test_transform, resize, device, visual, fh, fw,
                           fix_resize, mode, conf_threshold=0.3, ienet=True, angle_range=angle)
            print(info)
