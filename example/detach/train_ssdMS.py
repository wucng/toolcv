...
from toolcv.network.net import SSDsimpleMS
from toolcv.tools.utils import train_ssdMS as train, predict_ssdMS as predict
from toolcv.tools.anchor import get_prior_boxMS  # getAnchorsV2_s
...
class FruitsNutsDataset(Dataset):
    ...
    def __init__(self, root="", classes=[], resize=416, strides=[8, 16, 32], muilscale=False,
                 ...
                 use_mosaic=False, fix_resize=False, anchors=[],
    ...
    def __getitem__(self, idx):
        if self.muilscale:
            ...
            img, featureMap = getitem_mutilscale_ssdMS(self, idx, self.batch_size, self.strides, self.mode,
                                                       self.use_mosaic, self.fix_resize, self.anchor,
...
def main(training=True, visual=True):
    ...
    anchors = get_prior_boxMS((resize, resize), strides, xywh=False)  # 缩放到0~1
    num_anchors = 6
    ...
    model = SSDsimpleMS('dla34', True, num_anchors, num_classes, strides).to(device)
    ...
    if training:
        ...
        # optim = torch.optim.SGD([parm for parm in model.parameters() if parm.requires_grad],
        #                             lr=5e-4, weight_decay=5e-5, momentum=0.95)
        ...
        for epoch in range(epochs):
            train(model, optim, dataLoader, device, epoch, 'sum', 20, focal_loss, 5.0, 0.1)
        ...
        for img_path in img_paths:
            info = predict(anchors, model, img_path, test_transform, resize, device, visual, strides, fix_resize,
                           'output', 0.3, 0.5, focal_loss)
            print(info)
