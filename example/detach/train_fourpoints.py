...
from toolcv.tools.utils import train_fourpoints as train, predict_fourpoints as predict
...
def main(training=True, visual=True):
    ...
    model = Yolov1simple('resnet34', True, num_anchors, num_classes, strides, 0.5, 9).to(device)
    ...
    if training:
        ...
        for img_path in img_paths:
            info = predict(model, img_path, test_transform, resize, device, visual, fh, fw,
                           fix_resize, mode, fourpoints=True, angle_range=angle,conf_threshold=0.3)
            print(info)
