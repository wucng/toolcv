...
from toolcv.tools.utils import train_ssd as train, predict_ssd as predict
...
def main(training=True, visual=True):
    ...
    model = SSDsimple('dla34', True, num_anchors, num_classes, strides).to(device)
    # model = SSDsimpleV2('dla34', True, num_anchors, num_classes, strides).to(device)
    ...
    if training:
        ...
        for img_path in img_paths:
            info = predict(anchors, model, img_path, test_transform, resize, device, visual, fh, fw, fix_resize,
                           'output', 0.3, 0.9, focal_loss)
            print(info)
