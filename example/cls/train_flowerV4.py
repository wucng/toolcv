...
from toolcv.cls.tools import FlowerPhotosDataset, FlowerPhotosDatasetV4
...
def train(model, optimizer, dataloader, criterion, device, epoch, log_interval=500):
    ...
    for idx, (data, label, true_class, test_image) in enumerate(dataloader):
        ...
        #        0.2 * F.cross_entropy(test_label, test_label.detach().argmax(1), reduction="none")

        loss = true_class * F.cross_entropy(predited_label, label, reduction="none") + 2 - \
               (1 - true_class) * F.mse_loss(predited_label.softmax(1),
                F.one_hot(label, predited_label.size(1)).float(),reduction="none").sum(1) + \
               0.1 * F.cross_entropy(test_label, test_label.detach().argmax(1), reduction="none")
...
train_dataset = FlowerPhotosDatasetV4(data_path, data_path, classes, train_transforms)
