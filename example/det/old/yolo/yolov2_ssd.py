...
使用 ssd的方式 计算 gt_boxes 与 先验 anchors（x1,y1,x2,y2） 的 IOU 筛选 正负样本 (**速度很慢**)
...
from toolcv.utils.tools.tools import glob_format, get_bboxes_yolov3 as get_bboxes
...
from toolcv.tools.utils import box_iou, wh_iou, xywh2x1y1x2y2, x1y1x2y22xywh
...
head = YOLOFHead(backbone.out_channels // 4, num_anchor, num_classes)
...
class YoloF(_BaseNetV2):
    ...
    def get_targets(self, imgs, targets, feat_shape, feat_dtype):
        ...
        for batch_id in range(bs):
            ...
            gt_bbox_norm = gt_bbox / torch.tensor([img_w, img_h, img_w, img_h], dtype=gt_wh.dtype)  # 统一缩放到0~1
            # 与先验anchor计算iou 分配正负样本(统一缩放到0~1)
            # ious = wh_iou(self.anchors, gt_wh) # yolo方式
            ious = box_iou(self.anchors, gt_bbox_norm)  # 使用 ssd 方式
            # 每个gt 对应的最大的先验anchor
            ...
            for j, index in enumerate(indexs):
                ...
                # 对应的先验anchor
                x1, y1, x2, y2 = self.anchors[index]
                a_w = x2 - x1
                a_h = y2 - y1
                a_x = (x1 + x2) / 2
                a_y = (y1 + y2) / 2
                ...
                # 通过anchor 反算 对应的 位置
                cy = index // (feat_w * num_anchor)
                cx = index % (feat_w * num_anchor) // num_anchor
                ca = index % (feat_w * num_anchor) % num_anchor
                # 使用 anchor 反算出来的heatmap位置
                ctx_int, cty_int = cx, cy
                n_anchor = ca
                ...
                out_targets[n_layer][batch_id, cty_int, ctx_int, n_anchor, 1] = (
                                                                                        ctx / feat_w - a_x) / a_w  # ctx - ctx_int
                out_targets[n_layer][batch_id, cty_int, ctx_int, n_anchor, 2] = (
                                                                                        cty / feat_h - a_y) / a_h  # cty - cty_int
                ...
                for idx, iou in enumerate(ious[:, j]):
                    if iou > 0.5 and idx != index.item():
                        # 反算 对应的 位置
                        cy = idx // (feat_w * num_anchor)
                        cx = idx % (feat_w * num_anchor) // num_anchor
                        ca = idx % (feat_w * num_anchor) % num_anchor
                        # 使用 anchor 反算出来的heatmap位置
                        ctx_int, cty_int = cx, cy
                        n_anchor = ca
    ...
    def train_step(self, batch, step):
        ...
        with torch.cuda.amp.autocast(self.use_amp):
            ...
            cls_score = cls_score.permute(0, 2, 3, 1).contiguous().view(bs, h, w, num_anchor, -1)
            bbox_pred = bbox_pred.permute(0, 2, 3, 1).contiguous().view(bs, h, w, num_anchor, -1)
            object_pred = object_pred.permute(0, 2, 3, 1).contiguous().view(bs, h, w, num_anchor, 1)
    ...
    def pred_step(self, imgs, iou_threshold, conf_threshold, scale_factors, padding, in_shape, with_nms):
        ...
        for j, output in enumerate(outputs):
            ...
            anchor_xywh = x1y1x2y22xywh(anchors).contiguous().view(1, fh, fw, num_anchor, 4)
            wh_preds = wh_preds.exp() * anchor_xywh[..., 2:]  # 0~1
            # 恢复到heatmap上
            wh_preds[..., 0] *= fw
            wh_preds[..., 1] *= fh

            offset_preds *= anchor_xywh[..., 2:]
            offset_preds += anchor_xywh[..., :2]
            offset_preds[..., 0] *= fw
            offset_preds[..., 1] *= fh
...
# -----------------------eval ---------------------------
network.evalute(mode='coco', conf_threshold=0.2, with_nms=False)
# # -----------------------predict ---------------------------
