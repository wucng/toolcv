...
from toolcv.utils.net.net import SPPv2, CBA
from toolcv.utils.net.basev2 import BaseNet
from toolcv.api.define.utils.model.net import Backbone, RpnHead, RoiHeadC4, \
    FasterRcnnBBoxHeadC4V2, _initParmas, YOLOFNeck, RoiHeadC5, FasterRcnnBBoxHeadC5
...
rpn = RpnHead(backbone.out_channels // 4, num_anchors)
roihead = RoiHeadC5(strides)
# bboxhead = FasterRcnnBBoxHeadC5(backbone.out_channels // 4, num_classes=num_classes)
bboxhead = nn.Sequential(
    CBA(backbone.out_channels // 4, backbone.out_channels // 2, 3),
    CBA(backbone.out_channels // 2, backbone.out_channels // 4, 3),
    CBA(backbone.out_channels // 4, backbone.out_channels // 2, 3),
    CBA(backbone.out_channels // 2, backbone.out_channels // 4, 3),
    nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(backbone.out_channels // 4, num_classes))
...
model = nn.Sequential(nn.Sequential(backbone, spp, neck), rpn, roihead, bboxhead).to(device)

model.forward_backbone = lambda x: model[0](x)
...
if train_rcnn:
    params += get_params(model[2:].modules(), lr, weight_decay, 0.6)
