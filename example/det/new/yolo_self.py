...
------------------------------------------------------------------------------
wh_offset_target_weight = object_target * (object_target > 0.3).float() # 9个点
 Average Precision  (AP) @[ IoU=0.50:0.95 | area=   all | maxDets=100 ] = 0.867
 Average Precision  (AP) @[ IoU=0.50      | area=   all | maxDets=100 ] = 0.992
 Average Precision  (AP) @[ IoU=0.75      | area=   all | maxDets=100 ] = 0.981

wh_offset_target_weight = object_target * (object_target > 0.4).float() # 5个点
...
from toolcv.utils.net.backbone import BackboneCspnet
...
lrf = 0.1
lr = 1e-3 #5e-4 if use_amp else 3e-3
weight_decay = 5e-6
epochs = 50
batch_size = 4
h, w = 640,640 #416, 416
...
# -------------------data-------------------
data = DataDefine(dir_data, classes, batch_size, resize, 0.8)
...
backbone = BackboneCspnet('cspdarknet53', True, stride=strides,freeze_at=2,norm_eval=False)
