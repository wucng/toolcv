...
from toolcv.utils.other.custom import EMA
...
class Common(nn.Module):
    ...
    def evalute(self, **kwargs):
        # eval前，apply shadow weights；eval之后，恢复原来模型的参数 (如果 训练与evalute 同时进行)
    ...
    def load_ckpt(self, ckpt_path):
        if os.path.exists(ckpt_path):
            ...
            if self.ema and 'ema' in ckpt:
                self.ema.model.load_state_dict(ckpt['ema'].float().state_dict())
    ...
    def save_ckpt(self, ckpt_path="model.ckpt", epoch=0):
        ...
        #         'ema': deepcopy(self.ema.model).half()})
