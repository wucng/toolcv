...
def get_bboxes(centerness_pred, cls_pred, wh_preds, offset_preds, with_nms,
               iou_threshold, conf_threshold, scale_factors, padding, in_shape, to_img=True, k=100,
               mode_xy="grid", mode_wh="img"):
    """
    centerness_pred:[bs,c,h,w] 已经 加上 sigmoid
    cls_pred:[bs,c,h,w]
    wh_preds:[bs,2,h,w]
    offset_preds:[bs,2,h,w]
    """
    ...
    img_size = torch.tensor([img_w, img_h], device=wh_preds.device)[None, None]

    if mode_xy == "grid":
        xy = (offset_preds + torch.stack((batch_xs, batch_ys), -1)) * stride
    elif mode_xy == "img":
        xy = offset_preds * img_size
    elif mode_xy == "stride":
        xy = offset_preds * stride
    else:
        xy = (offset_preds + torch.stack((batch_xs, batch_ys), -1)) * stride

    if mode_wh == "img":
        wh = wh_preds * img_size
    elif mode_wh == "stride":
        wh = wh_preds * stride
    else:
        wh = wh_preds * img_size
...
def get_bboxes_dynamic_anchor(anchors, object_pred, cls_pred, bbox_offset, wh_cls, with_nms,
                              iou_threshold, conf_threshold, scale_factors, padding, in_shape,
                              to_img=True, k=100, center_zoom=False):
    """动态选择 anchor"""
    ...
    for i in range(bs):
        ...
        for j in range(h):
            ...
            _anchors.append([anchors[:, 0][iw], anchors[:, 1][ih]])
    ...
    if center_zoom: offset_preds *= anchors
...
get_bboxes_DA = get_bboxes_dynamic_anchor


def get_bboxes_anchors(anchors, object_pred, cls_pred, bbox_offset, with_nms,
                       iou_threshold, conf_threshold, scale_factors, padding, in_shape,
                       to_img=True, k=100, center_zoom=False):
    """yolov3 方式选择 anchor"""

    n_anchors, fh, fw = object_pred.shape[1:]
    object_pred = _nms(object_pred, 3)
    # 选取前100 top_100
    batch_index, batch_anchor, batch_score, batch_ys, batch_xs = top_k(object_pred, k=k)
    cls_pred = transpose_and_gather_feat(cls_pred, batch_index)
    bs, c, h = cls_pred.shape
    cls_pred = cls_pred.contiguous().view(bs, c, n_anchors, -1)
    cls_pred = torch.stack([torch.stack([cls_pred[i, j, batch_anchor[i, j]] for j in range(c)], 0) for i in range(bs)],
                           0).to(cls_pred.device)
    cls_score, cls_label = cls_pred.max(-1)
    bbox_offset = transpose_and_gather_feat(bbox_offset, batch_index)
    bbox_offset = bbox_offset.contiguous().view(bs, c, n_anchors, -1)
    bbox_offset = torch.stack([torch.stack([bbox_offset[i, j, batch_anchor[i, j]] for j in range(c)], 0) for i in range(bs)],
                           0).to(cls_pred.device)

    # 找到对应的anchor
    anchors = torch.stack([torch.stack([anchors[batch_anchor[i, j]] for j in range(c)], 0) for i in range(bs)],
                           0).to(cls_pred.device)
