...
class BottleBlockXv1(nn.Module):
    """resneXt bottle block"""
    def __init__(self,in_c,hide_c,out_c,ksize=3,stride=1,use_se=False,cardinality=32):
        super().__init__()
        self.cardinality = cardinality
        layers = [nn.Sequential(
            CBA(in_c,hide_c,1,1),
            CBA(hide_c,hide_c,ksize,stride),
            CBA(hide_c, out_c, 1, 1, activate=False)
    ...
    def forward(self,x):
        x1 = sum([self.layers[i](x) for i in range(self.cardinality)])
        x1 = self.seblock(x1)
        x = self.downsample(x)

        return self.act(x+x1)

class BottleBlockXv2(nn.Module):
    """resneXt bottle block"""
    def __init__(self,in_c,hide_c,out_c,ksize=3,stride=1,use_se=False,cardinality=32):
        super().__init__()
        self.cardinality = cardinality
        ...
        self.layers = nn.Sequential(*layers)
        self.conv1x1 = CBA(hide_c*cardinality, out_c, 1, 1, activate=False)
        self.downsample = CBA(in_c,out_c,1,stride,activate=False) if stride>1 or in_c!=out_c else \
            nn.Sequential()
        self.act = nn.ReLU()
    ...
    def forward(self,x):
        x1 = torch.cat([self.layers[i](x) for i in range(self.cardinality)],1)
        x1 = self.conv1x1(x1)
        x1 = self.seblock(x1)
        x = self.downsample(x)
...
if __name__ == "__main__":
    ...
    m = resnet50(DGBottleneck,use_se=True)
    print(m)
    pred = m(x)
    print(pred.shape)
