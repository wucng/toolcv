...
def res2next50(in_c=3,num_classes=1000,dropout=0.0):
    """2+3*(3+4+6+3)=50"""
    layers = [3, 4, 6, 3]
    model = ResNet(in_c, BottleBlock, layers=layers, num_classes=num_classes, dropout=dropout)
    return model

def res2next101(in_c=3,num_classes=1000,dropout=0.0):
    layers = [3,4,23,3]
    model = ResNet(in_c, BottleBlock, layers=layers, num_classes=num_classes, dropout=dropout)
    return model

def res2next152(in_c=3,num_classes=1000,dropout=0.0):
    layers = [3,8,36,3]
    model = ResNet(in_c, BottleBlock, layers=layers, num_classes=num_classes, dropout=dropout)
    return model
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(),'res2next152.pth')