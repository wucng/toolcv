...
anchors = ssd512_anchors(featmap_sizes=featmap_sizes, clip=True, normalization=True)
...
head = SSDHead512(num_classes, num_anchors)
_initParmas(head.modules(), mode='normal')
model = nn.Sequential(backbone, head).to(device)


class SSD512(_BaseNetV2):
...
network = SSD512(model, num_classes, resize, anchors, strides, epochs, lr, weight_decay, lrf, warmup_iters=1000,
