...
class YoloF(_BaseNetV2):
    ...
    def train_step(self, batch, step):
        ...
        with torch.cuda.amp.autocast(self.use_amp):
            cls_pred, bbox_pred, centerness_pred = self.model(imgs)
            offset_pred = bbox_pred[:, :2]
            wh_pred = bbox_pred[:, 2:]

        bs, _, fh, fw = cls_pred.shape
        # get_target
        # target
    ...
    def pred_step(self, imgs, iou_threshold, conf_threshold, scale_factors, padding, in_shape, with_nms):
        cls_preds, bbox_preds, centerness_preds = self.model(imgs)
        offset_preds = bbox_preds[:, :2]
        wh_preds = bbox_preds[:, 2:]
