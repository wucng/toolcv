...
from toolcv.api.define.utils.model.base import _BaseNetV2, drawImg
from toolcv.api.define.utils.model.net import Backbone, FPN, RetinaHead, _initParmas
...
cfg = get_anchor_cfg()
anchors = None
# anchors = rcnn_anchors(cfg, 512, 512, clip=True, normalization=True)
...
epochs = 50
batch_size = 2
min_size, max_size = 800, 1330  # 最大边1333；最小边800
...
if use_mosaic:
    ...
    transforms_mosaic = aug.Compose([
        ...
        aug.RandomHorizontalFlip(),
        # aug.ResizeMax(*resize), aug.Padding(),
        aug.ResizeLimit(min_size, max_size),
...
# dataset = FruitsNutsDataset(dir_data, classes, transforms, 0, use_mosaic=False, transforms_mosaic=None)
...
train_dataset = FruitsNutsDataset(dir_data, classes, transforms, 0, use_mosaic=False, transforms_mosaic=None)
test_dataset = FruitsNutsDataset(dir_data, classes, test_transforms, 0, use_mosaic=False, transforms_mosaic=None)
# dataset.show(mode='cv2')
dataloader = LoadDataloader(train_dataset, test_dataset, 0.0, batch_size, {})
train_dataLoader, val_dataLoader = dataloader.train_dataloader(), dataloader.val_dataloader()

# ----------------model --------------------------
backbone = Backbone('dla34', True, num_out=3)
neck = FPN(backbone.out_channels, 256)
head = RetinaHead(256, 9, num_classes)
...
class RetinaNet(_BaseNetV2):
    ...
    def get_targets(self, imgs, targets, cls_shapes, reg_shapes):
        ...
        featmap_sizes = [(math.ceil(img_h / stride), math.ceil(img_w / stride)) for stride in self.strides]
        anchors = rcnn_anchors(cfg, img_h, img_w, featmap_sizes, clip=True, normalization=True)
        anchors = torch.cat(anchors, 0)
        anchors_xywh = x1y1x2y22xywh(anchors)
    ...
    def train_step(self, batch, step):
        ...
        imgs = aug.batch(imgs, 32).to(self.device)
        ...
        loss_centerness = GaussianFocalLoss()(cls_preds[..., -1].sigmoid()[keep], cls_targets[..., -1][keep],
    ...
    def pred_step(self, imgs, iou_threshold, conf_threshold, scale_factors, padding, in_shape, with_nms):
        ...
        anchors_xywh = (x1y1x2y22xywh(anchors).to(self.device)[None]).expand_as(reg_preds)
        # decode
        ...
        boxes, scores, labels = get_bboxes_ssdv2(featmap_sizes, num_anchors, centerness_preds, cls_preds, reg_preds,
                                                 with_nms, iou_threshold, conf_threshold,
...
network = RetinaNet(model, num_classes, resize, anchors, strides, epochs, lr, weight_decay, lrf, warmup_iters=1000,
...
network.predict(glob_format(dir_data), test_transforms, device, visual=False,
                conf_threshold=0.3, with_nms=True, method='resize')
