...
from toolcv.api.define.utils.model.net import Backbone, RpnHead, RoiHeadC4, FasterRcnnBBoxHeadC4V2, \
    MaskRoiHead, MaskHead, _initParmas
...
maskroiHead = MaskRoiHead(strides)
# maskHead = nn.Sequential(nn.Upsample(scale_factor=2), backbone.backbone[-1],
#                          MaskHead(backbone.out_channels, num_classes - 1))  # 只对正样本做
maskHead = MaskHead(backbone.out_channels // 2, num_classes - 1)  # 只对正样本做
...
model = nn.Sequential(backbone, rpn, roihead, bboxhead, maskroiHead, maskHead).to(device)
...
model.forward_maskroipool = lambda x, proposal: model[4](x, proposal)
model.forward_mask = lambda x: model[5](x)
...
class FasterRcnn(BaseNet):
    ...
    def get_proposal(self, imgs, targets, cls_preds, reg_preds, training=True):
        ...
        for i, _boxes in enumerate(boxes):
            ...
            if training:
                ...
                cls_target[keep] = gt_labels[indices].float()
                ...
                if keep.sum() > 0:
                    tmp_boxes = _boxes[keep]
                    gt_masks = gt_masks[indices]
                    roi_head = RoIAlign((14, 14), 1.0, 0, True)  # boxes 已经缩放到 masks尺度上 因此 缩放比例 spatial_scale=1.0
                    target_masks = roi_head(gt_masks[:, None], tmp_boxes.split(1, 0))
                    target_masks = F.interpolate(target_masks, scale_factor=2.0)

                    mask_targets.append(target_masks)
        ...
        return dict(proposal=proposal_list, scores=scores_list, labels=labels_list,
                    cls_targets=cls_targets, masks=mask_targets)
    ...
    def train_step(self, batch, step):
        ...
        if train_rcnn:
            ...
            if train_masks:
                if keep.sum() > 0:
                    maskroi_out = self.model.forward_maskroipool(c4, proposal)[keep]
                    mask_out = self.model.forward_mask(maskroi_out)
                    proposal_labels = cls_targets[keep].long()
                    proposal_masks = proposals_dict['masks']
                    proposal_masks = torch.cat(proposal_masks, 0).squeeze(1)

                    # 按label取出对应的
                    mask_out = torch.stack([mask_out[i, v.item()] for i, v in enumerate(proposal_labels)], 0)

                    loss_masks = F.binary_cross_entropy_with_logits(mask_out, proposal_masks, reduction='mean')
    ...
    def pred_step(self, imgs, **kwargs):
        ...
        preds['masks'] = preds['masks'][:, :, py1:h - py2, px1:w - px2]
        preds['masks'] = \
            F.interpolate(preds['masks'], scale_factor=(1 / scale_y, 1 / scale_x),
                          recompute_scale_factor=True)
        preds['masks'] = preds['masks'].squeeze(1)

        # preds['masks'][preds['masks'] < 0.2] = 0
        preds['masks'] *= (preds['masks'] > 0.5).float()
    ...
    def do_predict(self, imgs, iou_threshold, conf_threshold):
        ...
        if keep.sum() > 0:
            masks_ = []
            maskroi_out = self.model.forward_maskroipool(c4, [boxes])  # len(c4) == len([boxes])
            mask_out = self.model.forward_mask(maskroi_out).sigmoid()
            # 按label取出对应的
            mask_out = torch.stack([mask_out[i, v.item()] for i, v in enumerate(labels)], 0)
            # 对齐到输入图像上
            for i, (x1, y1, x2, y2) in enumerate(boxes):
                ...
                x2 = round(x2.item())
                y2 = round(y2.item())
                tmp_mask = F.interpolate(mask_out[i][None, None], (y2 - y1, x2 - x1))[0, 0]  # 恢复到目标框大小
        ...
        return dict(boxes=boxes, scores=scores, labels=labels, masks=masks_list[0])
...
network.predict(**dict(img_paths=glob_format(dir_data),
                       ...
                       conf_threshold=0.1, method=method, draw='draw_mask'))
