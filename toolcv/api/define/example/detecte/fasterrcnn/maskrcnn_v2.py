...
# RoiHeadC4 同时做 mask的 roipooling
maskHead = nn.Sequential(nn.Upsample(scale_factor=2), MaskHead(backbone.out_channels, num_classes - 1))  # 只对正样本做
...
model = nn.Sequential(backbone, rpn, roihead, bboxhead, maskHead).to(device)
...
model.forward_mask = lambda x: model[4](x)
...
class FasterRcnn(BaseNet):
    ...
    def train_step(self, batch, step):
        ...
        if train_rcnn:
            ...
            if train_masks:
                if keep.sum() > 0:
                    with torch.cuda.amp.autocast(self.use_amp):
                        mask_out = self.model.forward_mask(roi_out)[keep]
    ...
    def do_predict(self, imgs, iou_threshold, conf_threshold):
        ...
        mask_out = self.model.forward_mask(roi_out).sigmoid()
        ...
        if keep.sum() > 0:
            masks_ = []
            # mask_out = self.model.forward_mask(roi_out)
            # 按label取出对应的
