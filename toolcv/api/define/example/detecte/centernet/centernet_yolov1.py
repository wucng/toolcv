...
from toolcv.api.define.utils.tools.tools import get_bboxes, grid_torch, xywh2x1y1x2y2
...
class CenterNet(_BaseNetV2):
    ...
    def train_step(self, batch, step):
        ...
        bs, c, fh, fw = center_heatmap_pred.shape

        # get_target
        # target
        target_result, avg_factor, ratio = self.get_targets(imgs, targets, center_heatmap_pred.shape)
        center_heatmap_target = target_result['center_heatmap_target'].permute(0, 2, 3, 1).contiguous().view(bs, -1, c)
        ...
        center_heatmap_pred = center_heatmap_pred.permute(0, 2, 3, 1).contiguous().view(bs, -1, c)
        ...
        if ignore:
            ...
            for i in range(bs):
                ...
                if len(index) > 0:
                    for idx in index:
                        if center_heatmap_target[i][idx].sum() == 0: center_heatmap_target[i][idx] = -1  # 忽略

        # loss
        keep = center_heatmap_target != -1
        loss_center_heatmap = GaussianFocalLoss()(center_heatmap_pred.sigmoid()[keep], center_heatmap_target[keep],
