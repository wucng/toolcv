...
def get_trt_plugin(plugin_name):
    ...
    for plugin_creator in PLUGIN_CREATORS:
        if plugin_creator.name == plugin_name:
            lrelu_slope_field = trt.PluginField("neg_slope",
...
def populate_network(network, weights=None, ModelData=None):
    ...
    lrelu = network.add_plugin_v2(inputs=[input_layer], plugin=get_trt_plugin("Custom_LReLU_TRT"))
    lrelu.get_output(0).name = ModelData.OUTPUT_NAME
