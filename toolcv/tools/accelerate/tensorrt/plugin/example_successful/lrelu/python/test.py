...
def get_trt_plugin(plugin_name):
    ...
    for plugin_creator in PLUGIN_CREATORS:
        ...
        if plugin_creator.name == plugin_name:
            ...
            negSlope_field = trt.PluginField("negSlope", np.array([0.1], dtype=np.float32), trt.PluginFieldType.FLOAT32)
            field_collection = trt.PluginFieldCollection([negSlope_field])
...
def populate_network(network, weights=None, ModelData=None):
    ...
    relu = network.add_plugin_v2(inputs=[input_layer], plugin=get_trt_plugin("LRelu_TRT_2"))
