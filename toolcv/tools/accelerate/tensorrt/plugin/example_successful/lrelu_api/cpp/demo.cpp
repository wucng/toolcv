...
// initLibNvInferPlugins(&(gLogger.getTRTLogger()), "".c_str());
...
ICudaEngine* createEngine(IBuilder* builder, IBuilderConfig* config,SampleParams mParams)
{
    ...
    // ------------------------使用自定义plugin 实现relu------------------------------------------------------------------------
    float negSlope = 0.1f;
    auto creator = getPluginRegistry()->getPluginCreator("LReLU_TRT", "1");
    const PluginFieldCollection* pluginData = creator->getFieldNames();
    PluginField* fields = (PluginField*)pluginData->fields;
    fields[0].name = "negSlope";
    fields[0].type =  nvinfer1::PluginFieldType::kFLOAT32;
    fields[0].data = &negSlope;
    IPluginV2 *pluginObj = creator->createPlugin("LReLU_TRT", pluginData);
