...
namespace nvinfer1
{
    ...
    const char *PLUGIN_NAME{"Clip_TRT_2"};
    ...
    // LeakyReLU {{{
    ReluPlugin::ReluPlugin(float mClipMin, float mClipMax): mClipMin(mClipMin),mClipMax(mClipMax)
    {
    }
    ...
    {
        ReluPlugin *p = new ReluPlugin(mClipMin,mClipMax);
    ...
    */
    __global__ void relu_kernel(const float *input, float *output, int num_elem,float mClipMin, float mClipMax) {
        ...
        output[idx] = input[idx] > mClipMax ? mClipMax : (input[idx] < mClipMin ? mClipMin: input[idx]);

        // output[idx] = min<float>(max<float>(input[idx], mClipMin), mClipMax);
    }

    void ReluPlugin::forwardGpu(const float *const * inputs, float* output, cudaStream_t stream, int batchSize,float mClipMin, float mClipMax) {
        int block_size = thread_count_;
        int grid_size = (input_size_ * batchSize + block_size - 1) / block_size;
        relu_kernel<<<grid_size, block_size,0,stream>>>(inputs[0], output, input_size_ * batchSize,mClipMin,mClipMax);
    }
    ...
    int ReluPlugin::enqueue(int batchSize, const void*const * inputs, void** outputs, void* workspace, cudaStream_t stream)
    {
        ...
        forwardGpu((const float *const *)inputs, (float*)outputs[0], stream, batchSize,mClipMin,mClipMax);
        return 0;
    }
    ...
    {   
        ...
        assert(strcmp(fields[0].name, "clipMin") == 0);
        assert(fields[0].type == PluginFieldType::kFLOAT32);
        clipMin = *(static_cast<const float*>(fields[0].data));

        assert(strcmp(fields[1].name, "clipMax") == 0);
        assert(fields[1].type == PluginFieldType::kFLOAT32);
        clipMax = *(static_cast<const float*>(fields[1].data));

        ReluPlugin* obj = new ReluPlugin(clipMin,clipMax);
