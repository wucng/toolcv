...
class DefModelData:
    ...
    NP_DTYPE = np.float32
    INPUT_SHAPE = [3, 1, 1] # wight 3x5 ;bias 5
    OUTPUT_SIZE = [-1, 5]
...
def get_trt_plugin(plugin_name):
    ...
    for plugin_creator in PLUGIN_CREATORS:
        ...
        if plugin_creator.name == plugin_name:
            ...
            weight_field = trt.PluginField("weight", np.ones([3,5], dtype=np.float32).flatten(), trt.PluginFieldType.FLOAT32)
            bias_field = trt.PluginField("bias", np.ones([5], dtype=np.float32), trt.PluginFieldType.FLOAT32)

            field_collection = trt.PluginFieldCollection([weight_field, bias_field])
...
def populate_network(network, weights=None, ModelData=None):
    ...
    relu = network.add_plugin_v2(inputs=[input_layer], plugin=get_trt_plugin("FC_TRT_2"))
