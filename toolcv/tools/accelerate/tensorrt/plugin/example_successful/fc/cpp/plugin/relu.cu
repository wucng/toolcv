 ...
 * ReluPlugin
    ...
    * 
    * getOutputDimensions()
    * 指定输出的维度 不包含 batch 这个维度
    * 
    * serialize(void* buffer)
    * 序列化文件 接收到的参数或权重 写到 序列化文件
    * 
    * getSerializationSize() 
    * 指定写入到 序列化文件的 内存大小 与 serialize(void* buffer) 写入的内存大小相关
    * 
    * deserializePlugin() 调用 ReluPlugin(const void* buffer, size_t length)
    * 
    * ReluPlugin(const void* buffer, size_t length)
    * 反序列化文件 从序列化文件 获取写入的参数
    * 
    * enqueue()
    * 运算核心
*/
...
namespace nvinfer1
{
    const char *PLUGIN_NAME{"FC_TRT_2"}; // 通过该名字获取 plugin
    ...
    {  
        ...
        return  INSIZE*OUTSIZE * sizeof(float)+OUTSIZE * sizeof(float) + sizeof(int)+sizeof(int); // weigih + bias + INSIZE + OUTSIZE
    }
    ...
    */
    ...
    {   
        ...
        // 根据数据 动态获取长度
        INSIZE = len_weight/len_bias;
        OUTSIZE = len_bias;

        std::cout<<__LINE__<<","<<INSIZE<<","<<OUTSIZE<<std::endl;
