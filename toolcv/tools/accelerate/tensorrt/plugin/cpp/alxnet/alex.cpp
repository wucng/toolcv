...
// #include "fc.h"
//#include "plugin/fc/fc.h"
#include "relu.h"
// #include "plugin/mish/mish.h"
...
ICudaEngine* createEngine(IBuilder* builder, IBuilderConfig* config,SampleParams mParams)
{
    ...
    // ------------------------使用自定义plugin 实现relu------------------------------------------------------------------------
    auto creator = getPluginRegistry()->getPluginCreator("Relu_TRT", "1");
    const PluginFieldCollection* pluginData = creator->getFieldNames();
    IPluginV2 *pluginObj = creator->createPlugin("relu0", pluginData);
    ITensor* inputTensors[] = {conv1->getOutput(0)};
    auto relu1 = network->addPluginV2(&inputTensors[0], 1, *pluginObj);
    ...
    // ------------------------使用自定义plugin 实现 fc------------------------------------------------------------------------
    auto creator = getPluginRegistry()->getPluginCreator("Fc_TRT", "1");
    // const PluginFieldCollection* pluginData = creator->getFieldNames();
    PluginFieldCollection* pluginData = (PluginFieldCollection*)creator->getFieldNames();

    pluginData->nbFields=2; // 存放 weight和bias
    PluginField* fields = (PluginField*)pluginData->fields;
    fields->name = "weight";
    fields[0].length = weightMap["classifier.6.weight"].count;
    fields[0].type =  nvinfer1::PluginFieldType::kFLOAT32;
    fields[0].data = weightMap["classifier.6.weight"].values;

    fields[1].name = "bias";
    fields[1].length = weightMap["classifier.6.bias"].count;
    fields[1].type =  nvinfer1::PluginFieldType::kFLOAT32;
    fields[1].data = weightMap["classifier.6.bias"].values;
    

    IPluginV2 *pluginObj = creator->createPlugin("fc0", pluginData); // 调用 fc.cu 中 的createPlugin
    // IPluginV2 *pluginObj = creator->createPlugin("fc0", pluginData,weightMap["fc.weight"],weightMap["fc.bias"]); 
    ITensor* inputTensors[] = {relu7->getOutput(0)};
    auto fc3 = network->addPluginV2(&inputTensors[0], 1, *pluginObj);
    // ITensor* inputTensors_yolo[] = {conv81->getOutput(0), conv93->getOutput(0), conv105->getOutput(0)};
    // auto yolo = network->addPluginV2(inputTensors_yolo, 3, *pluginObj);
    // ----------------------------------------------------------------------------------------------------------------------------------------
