...
if __name__ == "__main__":
    ...
    # loss = SoftDiceLoss()(masks_pred,true_masks)
    loss1 = SoftDiceLoss()(F.softmax(masks_pred, dim=1).float(),F.one_hot(true_masks, n_classes).permute(0, 3, 1, 2).float())
    loss2 = SoftDiceLoss(apply_nonlin=torch.nn.Softmax(dim=1))(masks_pred,F.one_hot(true_masks, n_classes).permute(0, 3, 1, 2).float())
    print(loss1)
    print(loss2)